...
│   ├── views/
│   │   └── upload.html             # 首頁 HTML（由 controller 在 runtime 讀取後回傳）
│   └── utils/
...
| 套件 | 預期整合位置 | 用途 |
|------|-------------|------|
| `pdf-lib` | 新路由（如 POST `/pdf/sign`） | 在 PDF 頁面嵌入文字、圖片或數位簽章 |
| `@pdf-lib/fontkit` | 與 pdf-lib 搭配使用 | 嵌入自訂字型（中文字型支援） |
| `pdf-parse` | 新路由（如 POST `/pdf/parse`） | 從 PDF buffer 擷取純文字 |
| `deepl-node` | 新路由（如 POST `/pdf/translate`） | 呼叫 DeepL API 翻譯文字 |
| `swagger-jsdoc` | app 初始化階段 | 自動從 JSDoc 生成 OpenAPI 3.0 規格 |
| `zod` | 各路由 handler | request body / query params 驗證 |
